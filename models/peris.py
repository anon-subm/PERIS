...
class PERIS(nn.Module):
    def __init__(self, opt):
        ...
        self.trn_end_time = max(trntimes) # for validatiopn
    ...
    def encode_timebin(self, timebins): # encoding time bins using enhanced RNN operation    
        
        if len(timebins.shape) == 3:         
        
            flat_timebins = timebins.reshape(-1, timebins.shape[-1])        

            # With computation enhancement
        ...
        else: # Item-level IS prediction (with flatten items)           
    ...
    # Item simiarlity-based virtual featuring and labeling (Intrinsic scheme)
    def augment_int_feature(self, users, emb_users, items, emb_items, history, pack_times, times, time_bins, emb_eval_items, mode): # mode: trn/vld/tst                 
        ...
        # 2. Divide items based on the start time of the label peri
        mask = (pack_times != 0)
    ...
    def neighbor_feature_label(self, users, neighbors, neighbors_time, neighbors_freqbin, mode): 

        # 1. user - cand user similarity
        # 2. aggregate freq bins based on the simialrity with the feature mask
    ...
    def compute_loss(self, batch_data):
        ...
        virtlabels, user_timebins = self.augment_int_feature(users, emb_user, items, emb_item, history, pack_times, times, user_timebins, None, mode='trn')
    ...
    def predict_for_multiple_item(self, userinfo, cand_items, mode, item_timebins, neighbors, neighbors_time, neighbors_freqbin):        
        ...
        _, user_timebins = self.augment_int_feature(users, emb_user, items, emb_item, None, None, times, user_timebins, emb_cand_items, mode)
